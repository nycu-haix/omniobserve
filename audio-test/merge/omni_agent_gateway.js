...
(() => {
	...
	const DEFAULT_ROOM_NAME = APP_DEFAULTS.roomName || "omniobserveaudiotest0427";

	const DEFAULT_JITSI_URL = APP_DEFAULTS.jitsiUrl || `https://meet.jit.si/${DEFAULT_ROOM_NAME}`;

	const DEFAULT_API_BASE_WS_URL = APP_DEFAULTS.apiBaseWsUrl || "ws://localhost:8000";
	...
	const DEFAULT_MODE = normalizeMode(params.get("mode") || APP_DEFAULTS.mode || "public");

	const AUTO_JOIN = params.has("autoJoin") ? params.get("autoJoin") !== "false" : APP_DEFAULTS.autoJoin !== false;
	...
	function readInitialConfig() {
		const jitsiUrl = params.get("jitsiUrl") || window.AGENT_JITSI_URL || localStorageGet("omni.jitsiUrl", "") || APP_DEFAULTS.jitsiUrl || DEFAULT_JITSI_URL;
		...
		const roomName = normalizeRoomName(
			params.get("roomName") || window.AGENT_ROOM_NAME || localStorageGet("omni.roomName", "") || APP_DEFAULTS.roomName || roomNameFromJitsiUrl || roomNameFromPath || DEFAULT_ROOM_NAME
		);

		const userId = params.get("userId") || window.AGENT_USER_ID || localStorageGet("omni.userId", "") || APP_DEFAULTS.userId || getOrCreateAnonymousUserId();

		const displayName = params.get("displayName") || window.AGENT_DISPLAY_NAME || localStorageGet("omni.displayName", "") || APP_DEFAULTS.displayName || userId;

		const apiWsUrl = params.get("apiWsUrl") || window.AGENT_API_WS_URL || APP_DEFAULTS.apiWsUrl || "";
	...
	function getConfig() {
		const jitsiUrl = ui.jitsiUrlInput.value.trim() || DEFAULT_JITSI_URL;

		const roomName = normalizeRoomName(ui.roomNameInput.value.trim() || parseRoomNameFromUrl(jitsiUrl) || DEFAULT_ROOM_NAME);

		const userId = ui.userIdInput.value.trim() || getOrCreateAnonymousUserId();

		const displayName = ui.displayNameInput.value.trim() || userId || "anonymous";

		const apiWsUrl = ui.apiWsUrlInput.value.trim() || buildDefaultApiWsUrl(roomName, userId);
	...
	async function switchMode(nextMode) {
		...
		state.autoUnmuteOnceOnPublic = oldMode === "private" && nextMode === "public";
	...
	function updateUiState() {
		...
		ui.modeStatus.className = "omni-pill " + (state.mode === "public" ? "public" : "private");

		ui.jitsiStatus.textContent = state.jitsiJoined ? "joined" : "not joined";
		ui.jitsiStatus.className = "omni-pill " + (state.jitsiJoined ? "running" : "stopped");

		ui.jitsiMicStatus.textContent = state.jitsiMicMuted ? "muted" : "unmuted";
		ui.jitsiMicStatus.className = "omni-pill " + (state.jitsiMicMuted ? "stopped" : "running");

		ui.agentStatus.textContent = agentStatus.running ? `running / ${agentStatus.scope}` : "stopped";

		ui.agentStatus.className = "omni-pill " + (agentStatus.running ? agentStatus.scope : "stopped");
		...
		ui.currentInfo.textContent = `room=${config.roomName}, userId=${config.userId}, displayName=${config.displayName}`;
			...
			while (offset + SEND_CHUNK_SIZE <= merged.length) {
				...
				if (this.ws && this.ws.readyState === WebSocket.OPEN) {
					...
					if (this.sentChunkCount % 100 === 0) {
						this.debug(`📤 sent ${this.scope} PCM chunks: ${this.sentChunkCount}, ws=${this.ws.readyState}`);
					}
				}
	...
	class JitsiIframeController {
		constructor({ getConfig, log, debug, onJoined, onLeft, onAudioMuteStatusChanged }) {
			this.getConfig = getConfig;
			this.log = log;
			this.debug = debug;
		...
		async join() {
			...
			await loadScriptOnce(`https://${parsed.domain}/external_api.js`, "jitsi-external-api-script");
	...
	function buildDefaultApiWsUrl(roomName, participantId) {
		...
		return `${base}/sessions/${encodeURIComponent(roomName)}` + `/audio-stream?participant_id=${encodeURIComponent(participantId)}`;
	}

	function sourceForScope(scope) {
		return normalizeMode(scope) === "private" ? "browser_private" : "browser_public";
	}

	function agentTypeForScope(scope) {
		return normalizeMode(scope) === "private" ? "private_browser" : "public_browser";
	}
	...
	function parseJitsiUrl(url, fallbackRoomName) {
		try {
			...
			const roomName = normalizeRoomName(parsed.pathname.replace(/^\/+/, "").split("/")[0] || fallbackRoomName || DEFAULT_ROOM_NAME);
	...
	function parseRoomNameFromUrl(url) {
		try {
			...
			return normalizeRoomName(parsed.pathname.replace(/^\/+/, "").split("/")[0]);
		} catch {
			return "";
		}
	}
