...
(() => {
  ...
  const DEFAULT_JITSI_URL =
    APP_DEFAULTS.jitsiUrl || `https://meet.jit.si/${DEFAULT_ROOM_NAME}`;
    ...
    handleBackendMessage(event) {
      ...
      try {
        ...
        if (data.type === "segment_saved") {
          const startText =
            typeof data.start === "number" ? data.start.toFixed(2) : data.start;
          const endText =
            typeof data.end === "number" ? data.end.toFixed(2) : data.end;

          this.log(
            `💾 segment saved: ${data.displayName || data.userId || this.getConfig().displayName}, ` +
            `${startText}s-${endText}s, reason=${data.reason || "unknown"}`
          );
          return;
        }
      ...
      this.processor.onaudioprocess = (event) => {
        ...
        if (SHOW_RMS) {
          ...
          if (now - this.lastLevelLogTime > 1000) {
            ...
            this.log(
              `🎙️ ${this.scope} pcm rms=${rms.toFixed(6)}, ` +
              `peak=${peak.toFixed(6)}, chunks=${this.sentChunkCount}`
            );
          }
        }
