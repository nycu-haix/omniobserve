class FrontendSileroVad {
	constructor(options = {}) {
		...
		this.minSpeechChunks = Math.ceil(((this.minSpeechMs / 1000) * this.sampleRate) / this.frameSize);

		this.maxSpeechChunks = Math.ceil(((this.maxSpeechMs / 1000) * this.sampleRate) / this.frameSize);

		this.minSilenceChunks = Math.ceil(((this.minSilenceMs / 1000) * this.sampleRate) / this.frameSize);

		this.preBufferChunks = Math.ceil(((this.preBufferMs / 1000) * this.sampleRate) / this.frameSize);
	...
	processAudio(pcm16kFloat32) {
		...
		const input = pcm16kFloat32 instanceof Float32Array ? pcm16kFloat32 : new Float32Array(pcm16kFloat32);
	...
	async predictFrame(frame) {
		...
		const inputTensor = new ort.Tensor("float32", modelInput, [1, this.effectiveFrameSize]);
		...
		this.context = modelInput.slice(this.effectiveFrameSize - this.contextSize, this.effectiveFrameSize);
	...
	updateSpeechState(frame, probability) {
		if (!this.speechStarted) {
			...
			if (probability >= this.startThreshold) {
				...
				const startTimeSec = Math.max(0, this.totalProcessedSamples / this.sampleRate - (this.preBuffer.length * this.frameSize) / this.sampleRate);
				...
				this.onDebug(`Silero start: segment=${segmentId}, prob=${probability.toFixed(4)}`);
			}
		...
		if (isSilenceEnd || isMaxLengthEnd) {
			...
			this.onDebug(`Silero end: segment=${segmentId}, reason=${endReason}, prob=${probability.toFixed(4)}`);
		}
	}
}
