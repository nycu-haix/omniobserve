...
function parseJitsiUrl(url) {
  ...
  const roomName = parsed.pathname
    .replace(/^\/+/, "")
    .split("/")[0]
    .toLowerCase();
...
function buildJitsiOptions() {
  ...
  if (serverConfig.websocket) {
    ...
    serviceUrl =
      `wss://${JITSI_DOMAIN}/xmpp-websocket?room=${encodeURIComponent(ROOM_NAME)}`;
  }
  ...
  conference.on(
    ...
    (id, participant) => {
      const displayName =
        ...
        participant?.getProperty?.("displayName") ||
        "unknown";
...
function onTrackAdded(track) {
  ...
  if (oldPipeline && oldPipeline.mediaStreamTrackId === newTrackId) {
    debug(
      `Duplicate audio track ignored: participant=${participantId}, trackId=${newTrackId}`
    );
    return;
  }
  ...
  log(
    ...
    `name=${displayName}, trackId=${newTrackId}`
  );
...
function startRemoteAudioPipeline(track) {
  ...
  if (oldPipeline) {
    ...
    log(
      `🧹 Cleaning old pipeline for participant=${participantId}, ` +
      `oldTrackId=${oldPipeline.mediaStreamTrackId}, newTrackId=${newTrackId}`
    );
  ...
  audioElement.play().then(() => {
    ...
    try {
      if (typeof audioElement.captureStream === "function") {
        ...
        log(
          `✅ createMediaElementSource connected to WebAudio mixer: ` +
          `participant=${participantId}`
        );
      }
    } catch (err) {
