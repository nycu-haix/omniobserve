...
import {
  ...
  MOCK_IDEA_BLOCKS,
  MOCK_SIMILARITY_CUES,
  MOCK_TRANSCRIPT_LINES,
} from "../../mock/privateBoard";
import type {
  BoardTab,
  IdeaBlock,
  SimilarityCueData,
  TranscriptLine as TranscriptLineType,
} from "../../types";
...
export function PrivateBoard({ roomId }: PrivateBoardProps) {
  ...
  const [transcriptLines, setTranscriptLines] = useState<TranscriptLineType[]>(
    ENABLE_PRIVATE_BOARD_MOCK_DATA ? MOCK_TRANSCRIPT_LINES : [],
  );
  const [highlightedBlockId, setHighlightedBlockId] = useState<string | null>(null);
  const [cues, setCues] = useState<SimilarityCueData[]>(
  ...
  const toggleBlock = (id: string) => {
    setIdeaBlocks((prev) =>
      prev.map((block) => (block.id === id ? { ...block, expanded: !block.expanded } : block)),
    );
  };
  ...
  return (
    <>
      <section className="flex h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-lg border bg-card text-card-foreground">
        <header className="flex items-center justify-between gap-3 border-b p-3">
          <div className="flex rounded-lg bg-muted p-1">
            ...
            <Button
              variant={activeTab === "ideablock" ? "secondary" : "ghost"}
        ...
        <ScrollArea className="min-h-0 flex-1 p-3">
          {activeTab === "ideablock" ? (
            <div className="grid gap-2">
              {ideaBlocks.length === 0 && (
                <div className="grid min-h-40 place-items-center rounded-lg border border-dashed text-muted-foreground">
                  尚無想法
                </div>
              )}
      ...
      <SimilarityCue
        cues={cues}
        onJump={jumpToBlock}
        onDismiss={(cueId) => setCues((prev) => prev.filter((cue) => cue.id !== cueId))}
      />
    </>
  );
}
