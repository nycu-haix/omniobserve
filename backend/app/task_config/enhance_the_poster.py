...
Private Phase 1 請從預設的海報元件與改善動作中組合至少 4 個 task items，沒有數量上限，並依照優先改善順序排列。進入 Private Phase 2 後，所有成員建立的 task items 會集中在一起；Private Phase 2 有 7 分鐘，請務必先完成自己的前 15 項排序。Public Phase 也只需要排序前 15 個最重要的改善項目，排在第 16 個之後的項目表示不會改動。Public Phase 可以開啟公開麥克風與其他人討論，且每位參與者請至少公開發言兩次。
...
Private Phase 2 有 7 分鐘。請務必在進入 Public Phase 前完成自己的前 15 項排序；如果時間快到，請先把你認為最重要的改善項目排到最前面。
